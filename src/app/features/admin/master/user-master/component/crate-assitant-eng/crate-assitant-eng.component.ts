...
@Component({
  ...
  templateUrl: './crate-assitant-eng.component.html',
  styleUrl: './crate-assitant-eng.component.scss'
})
export class CrateAssitantEngComponent {
  ...
  getDivisionList(cityId: any) {
    this.commonService.divisionList(cityId).subscribe((res:any) =>{
      ...
      if (this.userData) {        
        ...
        if (divisionValue) {
          let matchingCE;
          if (divisionValue.includes(",")) {
            let formatValue = divisionValue.split(",").map((val: any) => val.trim());
            matchingCE = this.divisionData.filter((zone: any) =>
              formatValue.includes(zone?.value.toString())
            );
          } else {
            matchingCE = this.divisionData?.find((zone: any) =>
              zone?.value.toString() === divisionValue.toString()
            );
          }
