...
@Component({
  ...
  templateUrl: './crate-supritending-eng.component.html',
  styleUrl: './crate-supritending-eng.component.scss'
})
export class CrateSupritendingEngComponent {
  ...
  getCircleList(zoneId: any) {
    this.circleList = [];
    this.commonService.circleList(zoneId).subscribe((res: any) => {
      this.circleList = res?.body?.result || [];
      if (this.userData) {
        let circleValue = this.userData?.circle;
        if (circleValue) {
          let matchingCE;
          if (circleValue.includes(",")) {
            let formatValue = circleValue.split(",").map((val: any) => val.trim());
            matchingCE = this.circleList.filter((zone: any) =>
              formatValue.includes(zone?.value.toString())
            );
          } else {
            matchingCE = this.circleList.find((zone: any) =>
  ...
  getZoneBasedOnCE(id: any) {
    this.commonService.getZoneByChiefEng(id).subscribe((res: any) => {
      ...
      if (this.userData) {
        let matchingCE = this.zoneList.find((zone: any) => zone?.value == this.userData?.zone);
        this.userForm.controls['zone'].setValue(matchingCE);
        this.getCircleList(matchingCE?.value);
